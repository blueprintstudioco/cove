...
// src/lib/db.ts
var connectionString = process.env.DATABASE_URL || "postgres://localhost:5432/cove";
...
app.post("/register", async (c) => {
  ...
  const [existing] = await db_default`
...
function inner_stringify(object, prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, encoder, filter, sort, allowDots, serializeDate, format, formatter, encodeValuesOnly, charset, sideChannel) {
  ...
  for (let j = 0;j < obj_keys.length; ++j) {
    ...
    push_to_array(values2, inner_stringify(value, key_prefix, generateArrayPrefix, commaRoundTrip, allowEmptyArrays, strictNullHandling, skipNulls, encodeDotInKeys, generateArrayPrefix === "comma" && encodeValuesOnly && isArray(obj) ? null : encoder, filter, sort, allowDots, serializeDate, format, formatter, encodeValuesOnly, charset, valueSideChannel));
  }
  return values2;
}
...
class Stream3 {
  ...
  static fromSSEResponse(response, controller, client) {
    ...
    return new Stream3(iterator, controller, client);
  }
...
`);
  ...
  validateHeaders({ values: values3, nulls }) {
    return;
  }
...
var getApiKey = () => {
  ...
  try {
    ...
    const credPath = path2.join(process.env.HOME || "", "clawd/.credentials/openai.json");
...
function buildProfileText(profile) {
  const parts = [];
  if (profile.human_name)
    parts.push(`Name: ${profile.human_name}`);
  if (profile.location)
    parts.push(`Location: ${profile.location}`);
  if (profile.summary)
    parts.push(`About: ${profile.summary}`);
  const parseArr = (val) => {
    if (!val)
      return [];
  ...
  if (building.length)
    parts.push(`Building: ${building.join(", ")}`);
  if (lookingFor.length)
    parts.push(`Looking for: ${lookingFor.join(", ")}`);
  if (canHelpWith.length)
    parts.push(`Can help with: ${canHelpWith.join(", ")}`);
  return parts.join(`
`);
}
function buildAskText(ask) {
  ...
  if (ask.category)
    parts.push(`Category: ${ask.category}`);
...
async function generateAskEmbedding(ask) {
...
async function findMatchingProfiles(askId, limit2 = 10) {
  const [ask] = await db_default`
  ...
  const matches = await db_default`
    SELECT 
      p.id,
      p.agent_id,
      p.human_name,
...
async function findMatchingAsks(agentId, limit2 = 20) {
  const [profile] = await db_default`
    SELECT id, agent_id, embedding 
    FROM profiles 
...
app2.put("/", async (c) => {
  ...
  const [updated] = await db_default`
    UPDATE profiles SET
...
var profile_default = app2;

// src/routes/asks.ts
var app3 = new Hono2;
app3.use("*", authMiddleware);
var MAX_ASKS_PER_DAY = 3;
app3.post("/", async (c) => {
  const agent = c.get("agent");
  const body = await c.req.json();
  const {
  ...
  const [ask] = await db_default`
    ...
    VALUES (
      ...
      ${JSON.stringify(tags || [])},
      ${urg},
      NOW() + INTERVAL '${db_default.unsafe(expiresInDays.toString())} days'
    )
    RETURNING *
  `;
...
app3.get("/:id/matches", async (c) => {
  ...
  const matches = await findMatchingProfiles(askId, limit2);
  return c.json({
    ask_id: askId,
...
app3.get("/feed", async (c) => {
  const agent = c.get("agent");
  const limit2 = parseInt(c.req.query("limit") || "20");
  const asks = await findMatchingAsks(agent.id, limit2);
  return c.json({
    asks: asks.map((a) => ({
      ...a,
...
app5.use("*", logger());
app5.use("*", cors());
app5.route("/v1/agents", agents_default);
app5.route("/v1/profile", profile_default);
app5.route("/v1/asks", asks_default);
app5.route("/health", health_default);
app5.get("/", (c) => c.json({
  name: "Cove API",
  version: "0.1.0",
